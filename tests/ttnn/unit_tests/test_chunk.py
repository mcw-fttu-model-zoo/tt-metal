...
def test_custom_chunk(device, input_tensor, num_chunks, dim):
    ...
    print("Torch Output", torch_chunks)
    print("TTNN Output", ttnn_chunks_torch)

    assert len(torch_chunks) == len(
        ttnn_chunks_torch
    ), f"Chunk counts differ: {len(torch_chunks)} vs {len(ttnn_chunks_torch)}"
